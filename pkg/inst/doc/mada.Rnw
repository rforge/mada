...
% 
% o Please make sure that all code/figures/tables/text are within the
% margins of the manuscript
% 
% Please make sure that: 
% 
% Code:
% Please make sure that all files needed to replicate the code/any examples within the manuscript are included in a standalone replication script.
...
Correlation of sensitivities and false positive rates: 
 ...
 0.677  0.228  0.888 
\end{Soutput}
For the AUDIT-C data, the underlying call to \code{prop.test} produces a warning which should not worry us here. The \code{madad} function has a range of options with respect to computational details; for example one can compute 80\% confidence intervals:
<<eval=FALSE>>=
...
summary(fit.smoking)
@
This output can be interpreted as follows: The $z$~value for the regression coefficient for the sensitivities is significant, indicating that the interviewer administered questionnaires offer a better sensitivity. Interestingly the point estimate for the  false-positive rates does not indicate any effect.
...
AIC(fit.smoking2)
@
The lower AIC value indicates, that the fit of the second model is better.

\section{Further development}
In the future \pkg{mada} will support the mixture approach of \citet{holling2011likelihood}.
