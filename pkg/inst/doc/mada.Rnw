...
\]
and by the same relation a confidence interval for the AUC can be computed from the confidence interval for $\theta$. The \texttt{mada} package also offers the \texttt{AUC} function to calculate the AUC of other SROC curves which uses the trapezoidal rule.
...
@
\end{center}
The summary estimates are well separated, though the confidence regions slightly overlap. It would nevertheless be safe to conclude that IAQ is a more reliable way to measure smoking than SAQ.

\subsection{Using \texttt{mada} for bivariate meta-regression}
We demonstrate diagnostic meta-regression also using the data of \cite{patrick1994validity}. We use the complete data set, which is loaded by
<<>>=
data(smoking)
@
The \texttt{data.frame} contains the same variables as the \texttt{SAQ} and \texttt{IAQ} subsets, but the type is coded by the fifth variable \texttt{type}:
<<>>=
...
summary(fit.smoking)
@
The $z$-value for the regression coefficient for the sensitivities is significant, indicating that the interviewer administered questionnaires offer a better sensitivity. Interestingly the point estimate for the  false-positive rates does not indicate any effect.

Once meta-regression is used, one can not reasonably plot SROC curves, since fixed values for the covariates would have to be supplied to do so. Also (global) AUC values do not make sense.

\subsection{Transformations beyond the logit}
All bivariate approaches explained so far use the conventional logit transformation. The \texttt{reitsma} function offers the parametric $t_\alpha$ family (\cite{doebler2012mixed}) of transformations as alternatives. The family is defined by
\[
...
AIC(fit.smoking1)
AIC(fit.smoking2) # fit improved
@

\section{Further development of \texttt{mada}}
In the future \texttt{mada} will support the mixture approach of \cite{holling2011likelihood}.
