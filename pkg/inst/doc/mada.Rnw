...
summary(fit.phm.het)
@
The estimation of $\theta$ results in an SROC curve (see \cite{walter2002properties}, \cite{jones2005summary} for more background on SROC methods); plotting this curve together with confidence bands obtained from the confidence interval of $\theta$ in the summary is simple (we also add the original data on ROC space with confidence regions and only plot a portion of ROC space):
\begin{center}
<<fig = TRUE>>=
